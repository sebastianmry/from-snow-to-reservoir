...
SAR water classification depends on the acquisition geometry (layover and shadow vary with the ascending or descending orbit). Mixing several orbits in one time series produces an artificial sawtooth. `extract_timeseries.py` therefore reduces S1 to **one** consistent relative orbit. Rather than choosing automatically, the orbit is **fixed per AOI** (`s1_anchor`: Enguri 2024-08-29, Zhinvali 2024-08-25). Only dates of that 12-day repeat phase (`ordinal % 12`) are kept, so only that orbit is downloaded (about 1/4 of the dates instead of all). A date then enters the series if **either** the whole catchment is fully imaged (`valid_px_pct` at least 90%) **or** the reservoir itself is fully observed (`reservoir_valid_pct` at least 95%). On the Enguri anchor orbit the SAR swath misses the eastern Svaneti headwaters on roughly 33 of 54 cycles (catchment coverage about 59%) while still imaging the western reservoir in full, so those cycles carry a valid `reservoir_area_km2` and are kept for the lake series; their basin-wide `water_km2` is set to `NaN` because it is not comparable across coverage. Genuinely partial scenes that also miss the lake are discarded. The result is a clean, geometrically consistent 12-day series (Enguri 51, Zhinvali 52 scenes).
