...
Screenshots taken in August 2026. The weekly auto-update keeps extending both series, so the live
app shows more scenes than these figures.
...
This matters because both reservoirs studied here sit directly next to the Russia-controlled territories of Abkhazia (Enguri) and the Tskhinvali region, also known as South Ossetia (Zhinvali). The Enguri complex is shared across that boundary, with the dam upstream in Samegrelo and the power station inside Abkhazia, so its operation depends on a fragile cross-boundary arrangement. Enguri and its downstream Vardnili cascade together supplied about 36% of Georgia's hydropower generation in the 2017/18 winter season (Lobjanidze et al., 2019). The utility Georgian Water and Power puts Zhinvali at about half of Tbilisi's drinking water supply, and the reservoir also carries a share of the city's power, from a catchment that reaches towards the Tskhinvali region. Ground access to these headwaters is limited and politically sensitive, so satellite monitoring is the practical way to track the snow, glacier and water that feed two reservoirs critical to Georgia's energy and water security.
...
* **Glaciers are not detected from the imagery.** The glacier outlines come from the Randolph Glacier Inventory v7. For the Greater Caucasus these outlines were digitised from Landsat imagery of 1999 to 2002, so the glacier mask predates the study window by more than twenty years. The Caucasus has seen substantial glacier retreat since then (Tielidze et al., 2025), so the fixed mask overstates today's ice extent and cannot track that change. HLS only classifies snow on or off those static polygons, and the optical snow and ice split is coarse, because shaded slopes, thin cloud and debris-covered ice all blur the boundary. No newer RGI release was available at the time of writing, although an independent high-resolution inventory of the range has since been published (Tielidze et al., 2026; see [Future work](#future-work)).
...
**Measuring the glaciers themselves, not just a frozen RGI outline.** RGI v7's Greater Caucasus polygons were digitised once from Landsat imagery of 1999 to 2002, so `bare_ice_km2` tracks melt inside a boundary that has not moved in over twenty years while the real glaciers have retreated substantially. One newer inventory does now exist: Tielidze et al. (2026) mapped the Greater Caucasus at high
resolution for 2025 and report a mean area loss of 1.8% per year since 2014, which would give the
project a mask roughly a quarter of a century closer to the study window than RGI v7. Swapping it in
is the cheap first step, but it is still a single static snapshot. Tracking the change itself needs a
repeatable glacier-mapping method:
...
- Tielidze, L. G., Nosenko, G. A., Nadaraia, A., Khromova, T. E., Kumladze, R. M., Clason, C. C., Elashvili, M., & Gadrani, L. (2026). A 2025 high-resolution glacier inventory of the Greater Caucasus reveals accelerated area loss. *Remote Sensing*, 18(9), Article 1441. https://doi.org/10.3390/rs18091441
